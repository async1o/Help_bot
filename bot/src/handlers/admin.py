...
class Admins:
    ...
    async def add_admin_2(message: Message, state: FSMContext):
        try: 
            await AdminRepository().update_roles(user_id=str(message.text), add=True, operator=False)

            await message.answer(text='Администартор успешно добавлен')
        except NotImplementedError:
            await message.answer(text='Перед добавлением нового администратора, убедитесь что он использовал бота')
    ...
    async def delete_admin_2(message: Message, state: FSMContext):
        try: 
            await AdminRepository().update_roles(user_id=str(message.text), add=False, operator=False)
...
class Operators:
    ...
    async def add_operator_2(message: Message, state: FSMContext):
        try: 
            await AdminRepository().update_roles(user_id=str(message.text), add=True, operator=True)

            await message.answer(text='Оператор успешно добавлен')
        except NotImplementedError:
            await message.answer(text='Перед добавлением нового оператора, убедитесь что он использовал бота')
        except Exception as e:
            await message.answer(text='Что-то пошло не так')
    ...
    async def delete_operator_2(message: Message, state: FSMContext):
        try: 
            await AdminRepository().update_roles(user_id=str(message.text), add=False, operator=True)
